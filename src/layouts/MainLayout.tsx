      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/80 backdrop-blur-md">
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4" aria-label="Navegacion principal">
          <Link to="/" className="flex items-center gap-2 font-display text-xl text-white">
            <img src="/logo.png" alt="FARMEAR AURA" className="size-9 rounded-md object-contain" />
            FARMEAR AURA
          </Link>
          <div className="flex items-center gap-2">
            <NavLink to={isAuthenticated ? '/consultando' : '/login'} className={({ isActive }) => `rounded-md px-3 py-2 text-sm font-bold uppercase ${isActive ? 'bg-white/10 text-cyan-100' : 'text-slate-300 hover:text-white'}`}>
              Jugar
            </NavLink>
